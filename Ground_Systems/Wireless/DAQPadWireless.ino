...
String sd_fileName = "/data_hotfire2.csv";
...
void setup() {
  ...
  char dataHeader[300] = "Timestamp,";
  for(int i=0; i<NUM_PT; i++) sprintf(dataHeader + strlen(dataHeader), "PT%d,", i + 1);
  for(int i=0; i<NUM_LC; i++) sprintf(dataHeader + strlen(dataHeader), "LC%d,", i + 1);
  // for(int i=0; i<NUM_TC; i++) dataHeader += "T" + std::to_string(i+1) + (i==NUM_TC-1 ? "\n": ",");
  appendFile(SD, sd_fileName.c_str(), dataHeader);
